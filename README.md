...
生年月日はブラウザ内でのみ使用し、サーバー、URL、localStorage、sessionStorageへ保存しません。sessionStorageには日干支・サイクル番号とルールの版だけを保存。タブ内の再読み込みに対応し、不正・旧形式データは無視します。保存領域が使用不可でも同一クライアント遷移中はメモリから結果を表示できます。共有は一般的なタイプ紹介URLと、誕生日や通変星（アイテム）を含まない画像・文章を使用します。タイプ紹介ページの共有文は「私は〜でした」ではなく、タイプの紹介として書きます。
