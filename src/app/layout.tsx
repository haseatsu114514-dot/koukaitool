...
export const metadata: Metadata = {
  ...
  description: "生年月日から出会う、10のやさしいキャラクター。四柱推命の日干をヒントに、あなたらしさと隠れた強みを見つける簡略診断。",
  icons: { icon: assetPath("/favicon.svg") },
  robots: { index: true, follow: true },
};
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return <html lang="ja" data-scroll-behavior="smooth"><body><a className="skip-link" href="#main">本文へスキップ</a>
    ...
    <footer className="site-footer"><Link className="footer-brand" href="/">ステラファイル <span>STELLA FILE</span></Link><p>自分を決めつけるためではなく、<br className="mobile-only" /> 自分をちょっと好きになるために。</p><div><Link href="/about/">診断について</Link><Link href="/privacy/">プライバシー</Link><span>© STELLA FILE</span></div></footer>
  </body></html>;
}
