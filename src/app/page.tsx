...
import { CHARACTER_TYPES, elementColors } from "@/data/types";
...
export default function Home() {
  ...
  return <>
    ...
    <div className="parade" aria-hidden="true"><div className="parade-track">{parade.map((type, i) => { const element = elementColors(type.stem); return <span key={i} className="parade-item"><span className="parade-face" style={{ background: element.tint, "--el": element.color } as React.CSSProperties}><Character type={type} /></span>{type.displayName}</span>; })}</div></div>
    <section className="section page-width reveal"><div className="section-heading"><div><h2>10のタイプ</h2><p>ほめ待ちグリズリー、ちゃっかりうさぎ、温泉カピバラ……あなたはどれ？</p></div><Link className="text-link" href="/types/">すべて見る <ArrowRight size={17} /></Link></div><div className="preview-grid">{[0, 1, 7, 9].map(i => <TypeCard key={i} type={CHARACTER_TYPES[i]} />)}</div></section>
    <section className="how-section"><div className="page-width how-inner reveal"><div><h2>診断の流れ</h2><Link className="text-link" href="/about/">診断のしくみ <ArrowRight size={17} /></Link></div><ol className="steps"><li><span>1</span><div><h3>生年月日を入力</h3><p>生まれた時間はいりません。</p></div></li><li><span>2</span><div><h3>タイプがわかる</h3><p>性格や相性に加えて、あなただけのアイテムもわかります。</p></div></li><li><span>3</span><div><h3>友だちと見せ合う</h3><p>結果は画像で保存できます。</p></div></li></ol></div></section>
