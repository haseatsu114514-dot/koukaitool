...
async function exportCard(type: CharacterType) {
  ...
  ctx.textAlign = "center"; ctx.fillStyle = "#e8b95a"; ctx.font = '26px sans-serif'; ctx.fillText("私のタイプは", 540, 94);
  ...
  ctx.font = '20px sans-serif'; ctx.fillText("生年月日でわかる10タイプ性格診断", 540, 1265);
...
export function ShareActions({ type }: { type: CharacterType }) {
  ...
  const shareData = () => ({ title: `ステラファイル｜${type.displayName}`, text: `私は「${type.displayName}」でした！ ${type.shortCatch}\n#ステラファイル`, url: `${window.location.origin}${assetPath(`/types/${type.slug}/`)}` });
  ...
  return <div className="share-actions"><div className="share-buttons"><button className="button secondary" onClick={save} disabled={saving}><Download size={16} />{saving ? "保存中…" : "結果画像を保存"}</button><button className="icon-button" aria-label="結果をシェア" onClick={share}><Share2 size={18} /></button><button className="icon-button" aria-label="共有文をコピー" onClick={copy}><Copy size={18} /></button></div><p className="share-status micro" role="status">{message || ""}</p></div>;
}
