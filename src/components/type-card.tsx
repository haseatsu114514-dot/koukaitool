...
export function TypeCard({ type }: { type: CharacterType }) {
  return <Link className="type-card" href={`/types/${type.slug}/`}>
    ...
    <div className="type-card-body"><p className="type-card-no">No.{String(CHARACTER_TYPES.indexOf(type) + 1).padStart(2, "0")}</p><h3><Bx>{type.displayName}</Bx></h3>
    <ul className="card-tags">{type.keywords.map(word => <li key={word}>{word}</li>)}</ul></div>
  </Link>;
}
