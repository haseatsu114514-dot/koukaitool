...
function ItemCard({ tenGod }: { tenGod: TenGod }) {
  ...
  return <section className="item-card"><div className="item-icon"><Icon size={34} strokeWidth={1.5} aria-hidden="true" /></div><div><span className="insight-label">あなたのアイテム</span><h2>{copy.item}</h2><h3>{copy.title}</h3><p>{copy.strength}</p><p className="gentle-tip">{copy.hint}</p></div></section>;
}
